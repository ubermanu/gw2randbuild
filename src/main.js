...
export async function getDetailedProfessions() {
  return await fetchJson(
    `https://api.guildwars2.com/v2/professions?ids=${professions.join(',')}`
  )
}
...
export async function getDetailedLegends() {
  return await fetchJson(
    `https://api.guildwars2.com/v2/legends?ids=${legends.join(',')}`
  )
}
...
async function getDetailedSkills(ids) {
  return await fetchJson(
    `https://api.guildwars2.com/v2/skills?ids=${ids.join(',')}`
  )
}

async function getDetailedSpecializations(ids) {
  return await fetchJson(
    `https://api.guildwars2.com/v2/specializations?ids=${ids.join(',')}`
  )
}

async function getDetailedTraits(ids) {
  return await fetchJson(
    `https://api.guildwars2.com/v2/traits?ids=${ids.join(',')}`
  )
}
...
export async function generate() {
  ...
  const specializations = await getDetailedSpecializations(
  ...
  if (profession !== 'Revenant') {
    ...
    let skills = await getDetailedSkills(
